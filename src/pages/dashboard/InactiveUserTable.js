...
import { followUser } from 'store/reducers/user';
    ...
    <React.Fragment>
      <TableRow sx={{ '& > *': { borderBottom: 'unset' } }}>
        ...
        <Stack direction="row" spacing={3}>
          <Avatar alt="Remy Sharp" src={userRow.profile_image} />
          <div>{row.name}</div>
          </Stack>
        </TableCell>
                  ...
                  <TableRow key={userRow.display_name}>
                    <TableCell align="right">
                      <Stack direction="row" spacing={1}>
                        <Chip onClick={()=> dispatch(followUser({user: user, user_id: userRow.user_id}))} label="Follow" color="success" variant="outlined" />
                      </Stack>
                      </TableCell>
                    </TableRow>
                </TableBody>
...
export default function CollapsibleTable(props) {
  ...
  let rows = []
  let activeUser = user.filter(x => x.follow === false);
